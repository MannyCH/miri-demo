...
## Component Instances in Figma (CRITICAL)

When traversing a Figma frame, any node with `type === 'INSTANCE'` is a design system component. **Never recreate it with custom CSS.**

For every INSTANCE node encountered:
1. Look it up in `FIGMA_STORYBOOK_MAPPING.md` to find its Storybook equivalent
2. Use the Storybook component directly in code with the correct props
3. Do NOT extract its pixel values (padding, radius, color) and apply them as CSS — those styles are already correct inside the component

**Why this matters:** Figma exposes the exact computed values of every node, including component internals. Applying those values as CSS overrides will break the component's built-in states (hover, focus, disabled) and diverge from the design system.

**Stop condition:** If an INSTANCE node has no match in `FIGMA_STORYBOOK_MAPPING.md`, stop and ask before writing any custom markup.
